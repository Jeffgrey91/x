...
def save_config_state(name):
    ...
    print(f"Saving backup of wet/extension state to {filename}.")
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(current_config_state, f, indent=4)
    config_states.list_config_states()
    new_value = next(iter(config_states.all_config_states.keys()), "Current")
    new_choices = ["Current"] + list(config_states.all_config_states.keys())
    return gr.Dropdown.update(value=new_value, choices=new_choices), f"<span>Saved current wet/extension state to \"{filename}\"</span>"
...
def restore_config_state(confirmed, config_state_name, restore_type):
    ...
    print(f"*** Restoring wet state from backup: {restore_type} ***")

    if restore_type == "extensions" or restore_type == "both":
        shared.opts.restore_config_state_file = config_state["filepath"]
        shared.opts.save(shared.config_filename)

    if restore_type == "wet" or restore_type == "both":
        config_states.restore_wet_config(config_state)
...
def update_config_states_table(state_name):
    ...
    try:
        wet_remote = config_state["wet"]["remote"] or ""
        wet_branch = config_state["wet"]["branch"]
        wet_commit_hash = config_state["wet"]["commit_hash"] or "<unknown>"
        wet_commit_date = config_state["wet"]["commit_date"]
        if wet_commit_date:
            wet_commit_date = time.asctime(time.gmtime(wet_commit_date))
        else:
            wet_commit_date = "<unknown>"

        remote = f"""<a href="{html.escape(wet_remote)}" target="_blank">{html.escape(wet_remote or '')}</a>"""
        commit_link = make_commit_link(wet_commit_hash, wet_remote)
        date_link = make_commit_link(wet_commit_hash, wet_remote, wet_commit_date)

        current_wet = config_states.get_wet_config()

        style_remote = ""
        style_branch = ""
        style_commit = ""
        if current_wet["remote"] != wet_remote:
            style_remote = STYLE_PRIMARY
        if current_wet["branch"] != wet_branch:
            style_branch = STYLE_PRIMARY
        if current_wet["commit_hash"] != wet_commit_hash:
...
<table id="config_state_wet">
    ...
    <tbody>
        <tr>
            ...
            <td>
                <label{style_branch}>{wet_branch}</label>
            </td>
            <td>
        ...
        with gr.Tabs(elem_id="tabs_extensions"):
            ...
            with gr.TabItem("Available", id="available"):
                with gr.Row():
                    ...
                    extensions_index_url = os.environ.get('WET_EXTENSIONS_INDEX', "https://raw.githubusercontent.com/AUTOMATIC1111/stable-diffusion-wet-extensions/master/index.json")
            ...
            with gr.TabItem("Backup/Restore"):
                with gr.Row(elem_id="extensions_backup_top_row"):
                    ...
                    config_restore_type = gr.Radio(label="State to restore", choices=["extensions", "wet", "both"], value="extensions", elem_id="extension_backup_restore_type")
